...
public class StudyTypeService {
    ...
    public void updateStudyType(Long studyTypeID, StudyTypeDTO request) {
        StudyType studyType = studyTypeRepository.findById(studyTypeID)
                .orElseThrow(() -> new NotFoundException("A study type with the id " + studyTypeID + " does not exist."));
        studyType.setName(request.getName());
        studyType.setConsent(request.getConsent());
        studyTypeRepository.save(studyType);
    }
    ...
    public void deleteStudyType(Long studyTypeID) {
        studyTypeRepository.delete(studyTypeRepository.findById(studyTypeID)
                .orElseThrow(() -> new RuntimeException("A study type with the id " + studyTypeID + " does not exist.")));
    }
}
